"""
Calculations API router for Four-Engine System Architecture.

This module provides endpoints for executing CAL-* calculation functions
individually or in batches for testing and development purposes.
"""
...
from .debug import add_trace_entry
...
def dict_to_calculation_state(state_data: Dict[str, Any]) -> CalculationState:
    ...
    In production, this would use proper schema validation.
    """
    try:
        # Create a basic CalculationState from the dictionary
        # This is simplified for testing - production would validate against full schema
...
async def get_available_calculations(user: ClerkUser = Depends(get_current_user)):
    """
    Get list of available calculations.

    Returns:
        AvailableCalculationsResponse with all registered CAL-* functions
    """
    try:
...
async def run_single_calculation(
    ...
    Args:
        request: CalculationRequest with calc_id, entity_id, state_data

    Returns:
        CalculationResponse with result and trace information
    """
...
async def run_batch_calculations(
    ...
    Args:
        request: BatchCalculationRequest with list of calculations

    Returns:
        BatchCalculationResponse with results for all calculations
    """
    start_time = datetime.utcnow()
    results = []
    errors = []
    ...
    for i, calc_request in enumerate(request.calculations):
        try:
            ...
            single_response = await run_single_calculation(calc_request, background_tasks, user)
...
async def run_scenario_calculations(
    ...
    Args:
        scenario_file: Name of scenario file (e.g., "basic_income_tax.json")
        calc_ids: Optional list of specific calculations to run (runs all if not specified)

    Returns:
        BatchCalculationResponse with scenario results
    """
    try:
        # Load scenario file
        scenario_path = f"backend/tests/test_scenarios/{scenario_file}"
        ...
        return await run_batch_calculations(batch_request, background_tasks, user)
