...
| `age`                   | int          | per-person    | Age in years (derived or provided). |
...
| `dependants_count`      | int          | per-person    | Number of dependants attributed to this person. |
| `dependants_age_bands`  | list/array   | per-person    | Optional: age bands (e.g. `[0–5, 6–12, 13–18]`) to model education costs etc. |
...
| `household_size`         | int          | per-household | Total number of people in the household. |
...
| `owner_entity_ids`        | list/array   | per-property | IDs of person(s)/entity(ies) owning the property. |
| `ownership_shares`        | list/array   | per-property | Ownership shares corresponding to `owner_entity_ids`. |
| `property_type`           | enum         | per-property | `home`, `investment_residential`, `commercial`, `land`. |
| `state_territory`         | enum         | per-property | State/territory where property is located. |
| `purchase_price`          | number       | per-property | Original purchase price. |
| `purchase_date`           | date         | per-property | Acquisition date. |
| `current_valuation`       | number       | per-property | Current estimated market value. |
| `land_value`              | number       | per-property | Land value, for land tax. |
...
| `strata_body_corporate`   | number       | per-property | Annual strata/body corporate fees. |
| `land_tax`                | number       | per-property | Annual land tax (if known or estimated). |
| `building_insurance`      | number       | per-property | Annual building insurance. |
| `landlord_insurance`      | number       | per-property | Annual landlord insurance. |
| `repairs_maintenance`     | number       | per-property | Annual repairs & maintenance allowance. |
| `property_management_fees`| number       | per-property | Annual property management fees. |
| `other_property_costs`    | number       | per-property | Other recurring property costs. |
| `capital_works_deduction_rate` | number  | per-property | Div 43 rate applied to building cost (simplified). |
| `plant_equipment_pool_balance` | number  | per-property | Remaining plant & equipment pool for depreciation (simplified). |
| `acquisition_costs`       | number       | per-property | Stamp duty, legal fees, inspections, lender fees, etc. |
| `sale_costs_estimate`     | number       | per-property | Estimated selling costs (agent, marketing, legal). |
...
| `lvr_current`                | number       | per-loan   | Current loan-to-value ratio. |
